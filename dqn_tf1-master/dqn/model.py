...
class DQN(BaseModel):
    ...
    def _build_model(self):
        with tf.compat.v1.variable_scope("train"):
            self.S_in = tf.compat.v1.placeholder(
                tf.float32, (None, self.config.in_height, self.config.in_width,
        ...
        with tf.variable_scope("fixed"):
            self.fixed_S_in = tf.placeholder(
                tf.float32, (None, self.config.in_height, self.config.in_width,
                             self.config.history_length),
                name="state")
    ...
    def _q_net_cnn(self, state_in):
        ...
        for i, (num_filters, kernel_size,
                stride) in enumerate(self.config.cnn_archi):
            h = tf.layers.conv2d(
                inputs=h,
                filters=num_filters,
    ...
    def _loss_and_train_op(self):
        with tf.variable_scope("optimizer"):
            ...
            train_op = tf.train.RMSPropOptimizer(
                self.decayed_lr, decay=0.95, momentum=0.95,
