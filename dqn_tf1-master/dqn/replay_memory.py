...
class ReplayMemory:
    ...
    def _state(self, index):
        ...
        if index > self.history_length - 1:
            return self.screens[(index - (self.history_length - 1)):(
                index + 1), ...]
        else:
    ...
    def get_batch(self):
        ...
        while len(indexes) < self.batch_size:
            while True:
                ...
                if self.current <= index and index <= (
                        self.current + self.history_length - 1):
                    continue
                if self.dones[(index - self.history_length - 1):(
                        index - 1)].any():
                    continue
                break
    ...
    def testsave(self):
        ...
        for name, array in\
            zip(["screens", "actions", "rewards", "dones", "counts"],
                [self.screens, self.actions, self.rewards, self.dones, self.counts]):
            save_npy(array, os.path.join('save/test/', name))
